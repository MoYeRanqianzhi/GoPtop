...
- [2026-09-19 离线 AI 层（人机对战 + 实时胜率）](memory/2026-09-19-ai-offline-engines-and-winrate.md) — **Rapfi 是 GPL-3.0 与本项目不兼容**、围棋无宽松许可现成引擎须自建 MCTS；**wasm 时钟陷阱**（figrid 全程 std::time → 须 vendor + web-time；go_game_board 默认 feature 含 perf-event → 须 no-default-features，两者编译期均不报错）；playout 实测 wasm 3.2 万/s；五子棋胜率 logistic s=150（score 是「轮到走的一方」视角）；**分析必须走 Web Worker**；胜率卡红蓝单挑线 + 走势图，底部与「对局」卡三态切换
