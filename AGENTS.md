- Project name: GoPtop
- Core purpose: P2P Gomoku & Go Game, Rust Desktop/Mobile/Web App
...
Do NOT use the shell to run commands when a relevant dedicated tool is available. Using dedicated tools allows the user to better understand and review the work. To read files use the dedicated read tool instead of cat/head/tail. To edit files use the dedicated edit tool instead of sed/awk. To create files use the dedicated write tool instead of heredoc or echo redirection. To search for files use the dedicated glob tool instead of find or ls. To search file content use the dedicated grep tool instead of raw grep or rg. Reserve shell execution exclusively for system commands and terminal operations that require it.
...
Use the task-tracking tool to plan and track work. Mark each task completed as soon as it is done; do not batch.
...
Use workflow-style orchestration for task coordination — this is mandatory, not a preference. Maximize concurrency, trading token spend for development speed and quality. Delegate self-contained tasks to subagents to keep the main context clean; run repetitive parallel work through concurrent subagents. Never let independently-dispatched agents modify the same files: uncoordinated agents collide on shared files, so give every concurrent agent a mutually exclusive file scope.

Split review tasks into parallel dimensions (for example, a 12-way deep read of reference sources). Split fix tasks along file boundaries so each agent's file scope is mutually exclusive. Multiple orchestrations may run concurrently, but their file scopes must never overlap.

Every modifying agent completes its own full loop: edit the code, verify with cargo check / cargo test, and only after everything passes run git add + git commit; on failure, fix and retry until green — never commit red. The coordinating agent NEVER commits on a subagent's behalf. It coordinates, reports, and verifies landings through git log.

Standard-comparison audits (Skill, MCP, and similar industry-standard surfaces) are major undertakings that get a dedicated orchestration: first deep-read every reference file with high concurrency (one agent per file), then run structured comparison agents, then build a real test environment to verify actual usability. Compare against the unified industry standard and report every single deviation.
...
Memory format, spelled out in full since it follows Claude Code's memory conventions which other CLIs may not know: each entry in ./.agents/memory/ is a single Markdown file holding one topic. It opens with YAML frontmatter carrying `name` (a short kebab-case slug), `description` (a one-line summary used to judge relevance at recall time), and `metadata.type` (one of `user` / `feedback` / `project` / `reference`). The body states the fact; `feedback` and `project` entries follow the fact with **Why:** and **How to apply:** lines explaining the reasoning and the operational consequence. Related memories link to each other with `[[name]]` references. ./.agents/MEMORY.md is the index read first every session — one line per memory with a pointer and a short hook, never the full content. Before saving a new memory, check whether an existing file already covers the topic and update it rather than duplicating; delete memories that prove wrong. Do not record what the repository already records (code structure, git history, this file).
